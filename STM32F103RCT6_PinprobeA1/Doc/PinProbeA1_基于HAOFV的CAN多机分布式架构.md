...
输入锁存属于本机 IO 的派生状态，不替换 `RamVector` 中的原始 IO 事实：

- `ModBusTask` 仍是原始输入镜像的唯一 writer；`StateVector` 只读取本机快照。
- `InputState` 根据 Flash 中的输入点类型生成转发视图；`LEVEL` 点保持原始电平，只有 `PULSE` 点才允许锁存。
- 门和 USB 位置传感器的锁存置位/解除由 `StateVector` 提供方向限定的 `set_mask/clear_mask`：对应运动方向的首次消抖上升沿置位，反向运动再次出现上升沿解除；方向变化本身不清除锁存。
- 未提供应用路径的输入点保留原始电平，避免配置准备阶段改变急停、激光、气压等既有安全语义。
- `Risk Mode` 不拥有输入锁存策略，只读取本地气压传感器完成风险路径判定；气压判定不依赖转发视图。
- `READ:IO:ALL?` 的 `FORWARD` 结果是本机派生查询视图；CAN `IO_SUMMARY` 默认同步原始事实，后续如需跨节点使用锁存视图，应增加带版本的派生字段和 owner 定义。
